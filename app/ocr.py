...
# 获取图片ocr结果数据
async def result_with_binary_data(binary_data, file_name):
    # 初始化BytesIO字节串
    ...
    if len(origin_result) == 0:
        return final_result(200, file_name, -1, -1, [], [])
    # 计算最佳旋转角度
    ...
    if abs(angle_value) < 1:
        return final_result(200, file_name, angle_value, -1, origin_result, [])
    # 计算实际旋转角度
    ...
    return final_result(200, file_name, anticlosewise, -1, origin_result, rotated_result)
